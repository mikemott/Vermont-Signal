...
## WEEK 1: Foundation - Position Tracking & Proximity Co-occurrence

### Day 1-2: Database Schema Enhancement

#### Task 1.1: Create Database Migration Scripts

**File:** `scripts/migrations/001_add_position_tracking.sql`

```sql
-- Migration: Add position tracking to facts table
-- Purpose: Enable sentence-level proximity analysis
-- Date: 2025-01-XX
-- Rollback: See 001_rollback_position_tracking.sql

BEGIN;

-- Add position columns (nullable for backward compatibility)
ALTER TABLE facts
...
-- Add indexes for position-based queries
CREATE INDEX IF NOT EXISTS idx_facts_sentence
ON facts(article_id, sentence_index)
WHERE sentence_index IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_facts_paragraph
ON facts(article_id, paragraph_index)
WHERE paragraph_index IS NOT NULL;

-- Add comments for documentation
COMMENT ON COLUMN facts.sentence_index IS 'Zero-indexed sentence number within article (0 = first sentence)';
COMMENT ON COLUMN facts.paragraph_index IS 'Zero-indexed paragraph number within article';
COMMENT ON COLUMN facts.char_start IS 'Character offset of entity mention start in article content';
COMMENT ON COLUMN facts.char_end IS 'Character offset of entity mention end in article content';

COMMIT;
```

**File:** `scripts/migrations/001_rollback_position_tracking.sql`

```sql
-- Rollback script for position tracking migration
BEGIN;

DROP INDEX IF EXISTS idx_facts_sentence;
DROP INDEX IF EXISTS idx_facts_paragraph;
...
**File:** `scripts/migrations/002_enhance_relationships_table.sql`

```sql
-- Migration: Enhance entity_relationships for PMI and proximity
BEGIN;

-- Add new columns for relationship metadata
ALTER TABLE entity_relationships
ADD COLUMN IF NOT EXISTS pmi_score FLOAT,
ADD COLUMN IF NOT EXISTS npmi_score FLOAT,
ADD COLUMN IF NOT EXISTS raw_cooccurrence_count INTEGER,
ADD COLUMN IF NOT EXISTS proximity_weight FLOAT,
ADD COLUMN IF NOT EXISTS min_sentence_distance INTEGER,
ADD COLUMN IF NOT EXISTS avg_sentence_distance FLOAT;

-- Update relationship_type to support new categories
-- Current values: 'co-occurrence'
-- New values: 'same-sentence', 'adjacent-sentence', 'near-proximity', 'cross-article'

-- Add index on relationship strength for filtering
CREATE INDEX IF NOT EXISTS idx_relationships_npmi
ON entity_relationships(npmi_score DESC)
WHERE npmi_score IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_relationships_proximity
ON entity_relationships(article_id, proximity_weight DESC)
WHERE proximity_weight IS NOT NULL;

-- Comments
COMMENT ON COLUMN entity_relationships.pmi_score IS 'Pointwise Mutual Information score (unbounded)';
COMMENT ON COLUMN entity_relationships.npmi_score IS 'Normalized PMI score (range: -1 to 1)';
COMMENT ON COLUMN entity_relationships.raw_cooccurrence_count IS 'Total co-occurrence count (weighted by proximity)';
COMMENT ON COLUMN entity_relationships.proximity_weight IS 'Proximity-weighted co-occurrence (3=same sentence, 2=adjacent, 1=near)';
COMMENT ON COLUMN entity_relationships.min_sentence_distance IS 'Minimum sentence distance observed (0=same sentence)';
COMMENT ON COLUMN entity_relationships.avg_sentence_distance IS 'Average sentence distance across all co-occurrences';
...
# Run migrations on Hetzner production database
ssh root@159.69.202.29 "docker exec vermont-signal-postgres psql -U postgres -d vermont_signal_v2 -f /opt/vermont-signal/scripts/migrations/001_add_position_tracking.sql"

ssh root@159.69.202.29 "docker exec vermont-signal-postgres psql -U postgres -d vermont_signal_v2 -f /opt/vermont-signal/scripts/migrations/002_enhance_relationships_table.sql"

# Verify changes
ssh root@159.69.202.29 "docker exec vermont-signal-postgres psql -U postgres -d vermont_signal_v2 -c '\d facts'"
ssh root@159.69.202.29 "docker exec vermont-signal-postgres psql -U postgres -d vermont_signal_v2 -c '\d entity_relationships'"
```
...
**File:** `vermont_news_analyzer/modules/database.py`

**Modification 1: Update init_schema() method (around line 206-237)**
...
**Modification 2: Update store_extraction_result() method (around line 560-630)**

Find the `execute_values` call and REPLACE with:
...
```python
                execute_values(
                    ...
                    """
                    INSERT INTO facts (
                        article_id, extraction_result_id,
                        entity, entity_type, event_description,
                        confidence, source_models,
                        sentence_index, paragraph_index, char_start, char_end,
                        wikidata_id, wikidata_label, wikidata_description, wikidata_properties,
                        note
                    )
                    VALUES %s
                    ...
                    DO UPDATE SET
                        ...
                        source_models = ARRAY(
                            SELECT DISTINCT unnest(facts.source_models || EXCLUDED.source_models)
                        ),
                        -- Update positions if new data has them and old doesn't
                    ...
                    [
                        (
                            ...
                            fact.get('confidence'),
                            fact.get('sources', []),
                            fact.get('sentence_index'),  # NEW
                            fact.get('paragraph_index'),  # NEW
                            fact.get('char_start'),  # NEW
                            fact.get('char_end'),  # NEW
                            fact.get('wikidata_id'),
                            fact.get('wikidata_label'),
                            fact.get('wikidata_description'),
                            Json(fact.get('wikidata_properties')) if fact.get('wikidata_properties') else None,
                            fact.get('note', '')
                        )
...
#### Task 1.3: Create Position Tracker Module

**File:** `vermont_news_analyzer/modules/position_tracker.py`
        ...
        while True:
            ...
            if sentence_idx is not None:
                occurrences.append(EntityPosition(
                    entity=entity,
                    entity_type='UNKNOWN',  # Will be filled from entity_dict
...
**Unit Test File:** `tests/unit/test_position_tracker.py`
...
#### Task 1.4: Integrate Position Tracking into Main Pipeline

**File:** `vermont_news_analyzer/main.py` or relevant processing module

**Location:** In the main processing function (after validation, before enrichment)
...
**ADD this code block in the processing pipeline:**

```python
def process_article(article_data: Dict) -> Dict:
    """Process a single article through the full pipeline"""

    # ... existing code for ingestion, extraction, validation ...

    # NEW: Add position tracking
    logger.info("Tracking entity positions...")
    try:
        position_tracker = PositionTracker()
        validated_result.merged_facts = position_tracker.enrich_entities_with_positions(
            text=article_data['content'],  # Full article text
            entities=validated_result.merged_facts
        )
        logger.info(f"Position tracking complete: {len(validated_result.merged_facts)} entities enriched")
    except Exception as e:
        logger.warning(f"Position tracking failed, continuing without positions: {e}")
        # Continue processing - positions are optional

    # ... continue with enrichment and storage ...
```
...
#### Task 1.5: Create Proximity Matrix Builder

**File:** `vermont_news_analyzer/modules/proximity_matrix.py`
...
class ProximityMatrix:
    ...
    def build_matrix(
        ...
        co_matrix = defaultdict(lambda: CooccurrenceData(
            entity_a="",
            entity_b="",
            total_weight=0,
            occurrences=[],
            ...
            for entity_a in entities_i:
                ...
                for j in range(window_start, window_end):
                    ...
                    for entity_b in entities_by_sentence[sent_idx_j]:
                        ...
                        if not co_data.entity_a:  # First occurrence
                            co_data.entity_a = pair[0]
                            co_data.entity_b = pair[1]
